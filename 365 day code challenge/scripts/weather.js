...
$(document).ready(function() {
  ...
  $("#output").click(function() {
    var res = data.split('\n')
    var city = $('#city').val();
    var loc = search(city, res);
    if (loc != -1) {
      var nums = res[loc].split(/[a-z]+:*\W*/gi).map(n => parseFloat(n)).filter(n => n >= 0);
      var mean = findMean(nums).toString().match(/^-?\d+(?:\.\d{0,2})?/)[0];
      var variance = findVariance(nums).toString().match(/^-?\d+(?:\.\d{0,2})?/)[0];
...
function search(city, res) {
  for (var i = 0; i < res.length; i++) {
    if (res[i].includes(city))
      return i;
  }
  return -1;
}
